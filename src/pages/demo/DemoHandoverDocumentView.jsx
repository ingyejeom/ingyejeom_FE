...
export default function DemoHandoverDocumentView({ handoverId, onClose }) {
    ...
    useEffect(() => {
        const loadHandover = async () => {
            ...
            setIsLoading(true);
            try {
                const docRes = await api.get('/handover', { params: { id: handoverId } });
    ...
    const handleGeneratePdf = async () => {
        ...
        try {
            ...
            // In demo mode, just download locally
            pdf.save(fileName);
            alert('PDF가 다운로드되었습니다. (데모 모드)');
        } catch (error) {
    ...
    const renderModuleContent = (module) => {
        ...
        if (type === 'BASIC_INFO') {
            ...
            if (data.periodStart) fields.push({ label: '인계 시작일', value: data.periodStart });
            if (data.periodEnd) fields.push({ label: '인계 종료일', value: data.periodEnd });
            ...
            if (data.accessUrl) fields.push({ label: '접속 URL', value: data.accessUrl });
            if (data.accountId) fields.push({ label: '계정 ID', value: data.accountId });
            if (data.password) fields.push({ label: '비밀번호', value: data.password });
            if (data.permissionLevel) fields.push({ label: '권한 등급', value: data.permissionLevel });
            if (data.usageRule) fields.push({ label: '이용 규칙', value: data.usageRule, fullWidth: true });
        } else if (type === 'TASK') {
