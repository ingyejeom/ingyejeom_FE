...
export default function DemoHandover() {
    const navigate = useNavigate();
    const location = useLocation();
    const { id } = useParams();

    // Fixed demo space ID
    const targetSpaceId = '1';
    ...
    // Auto-generate default title for new documents (simplified for demo)
    useEffect(() => {
        ...
        <div style={styles.pageBackground}>
            <header style={styles.header}>
                <nav style={styles.breadcrumb}>
                    <span style={{ padding: '2px 8px', backgroundColor: '#FEF3C7', color: '#92400E', fontSize: '11px', fontWeight: '600', borderRadius: '4px', marginRight: '8px' }}>DEMO</span>
                    <span style={{ cursor: 'pointer', color: '#3B82F6' }} onClick={() => navigate('/demo/archive')}>자료실</span>
                    <span>&gt;</span>
                    <span>{metaInfo.groupName}</span>
                    <span>&gt;</span>
                    <span>{metaInfo.workName}</span>
                </nav>
